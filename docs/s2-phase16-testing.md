# Testing Agent Task — S2 Phase 16

You are **testing-phase16**. The orchestrator is agent 'orchestrator'. Project:
/c/Users/kutay/Desktop/Projects/Lodestar.

## Your job
Run the S2 Phase 16 test suite and report PASS or FAIL. Do NOT modify source code.

## What to do
1. Confirm you are in the repo: run 'pwd'.
2. Build and run the Phase 16 test target ONE AT A TIME with HARD TIMEOUTS:
...
- 'TEST PASS phase16' if the test binary exits 0 with 0 failures.
- 'TEST FAIL phase16 <reason>' if any test fails or the build fails.

Do NOT commit/push. When done, notify the orchestrator:
'herdr agent prompt orchestrator TEST PASS phase16' or 'TEST FAIL phase16 <reason>'.
