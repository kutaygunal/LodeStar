# Testing Agent Task — S2 Phase 13

You are **testing-phase13**. The orchestrator is agent 'orchestrator'. Project:
/c/Users/kutay/Desktop/Projects/Lodestar.

## Your job
Run the S2 Phase 13 test suite and report PASS or FAIL. Do NOT modify source code.

## What to do
1. Confirm you are in the repo: run 'pwd'.
2. Build and run the Phase 13 test target ONE AT A TIME with HARD TIMEOUTS:
...
- 'TEST PASS phase13' if the test binary exits 0 with 0 failures.
- 'TEST FAIL phase13 <reason>' if any test fails or the build fails.

Do NOT commit/push. When done, notify the orchestrator:
'herdr agent prompt orchestrator TEST PASS phase13' or 'TEST FAIL phase13 <reason>'.
